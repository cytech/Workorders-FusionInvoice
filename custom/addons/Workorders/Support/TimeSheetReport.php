...
class TimeSheetReport {
	...
	public function getResultsIIF( $fromDate, $toDate, $companyProfileId = null ) {
		...
		$invoices = Invoice::select( DB::raw( '"TIMEACT" AS TIMEACT' ),
			...
			DB::raw( 'NULL AS JOB' ),
			DB::raw( 'CONCAT_WS(", ",workorder_employees.last_name, workorder_employees.first_name) AS EMP' ),
			DB::raw( 'NULL AS ITEM' ),
		...
		if ( $companyProfileId ) {
			...
			$results['companyProfile_company'] = 'All Billing';
			$results['TSCompanyCreate'] = config('workorder_settings.tsCompanyCreate');
			$results['TSCompanyName'] = config('workorder_settings.tsCompanyName');
		}

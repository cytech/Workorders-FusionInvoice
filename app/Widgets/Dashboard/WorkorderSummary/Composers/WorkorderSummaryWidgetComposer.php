...
use Addons\Workorders\Models\WorkorderAmount;
...
class WorkorderSummaryWidgetComposer
{
    ...
    {
	    $addon = Addon::where('name', 'Workorders')->firstOrFail();
	    ...
	    if (!$addon->has_pending_migrations) {
		    ...
		    $view->with( 'workordersTotalDraft', 'Run Upgrade' )
		         ->with( 'workordersTotalSent', 'Run Upgrade' )
		         ->with( 'workordersTotalApproved', 'Run Upgrade' )
		         ->with( 'workordersTotalRejected', 'Run Upgrade' )
		         ->with( 'workorderDashboardTotalOptions', periods() );
	    }
    }
